...
export default function FinancePaging() {
    ...
    const moveButtonGroup = (newGroup) => {
        if (newGroup < 0 || newGroup > Math.floor((maxPage - 1) / PAGE_BUTTONS_PER_GROUP)) return;
    ...
    return (
        <>
            ...
            <div className="flex justify-center space-x-2 mt-6">
                ...
                <button
                    onClick={() => moveButtonGroup(buttonGroup + 1)}
                    disabled={buttonGroup === Math.floor((maxPage - 1) / PAGE_BUTTONS_PER_GROUP)}
                    className="px-3 py-1 bg-white/10 rounded cursor-pointer disabled:opacity-30"
                >
                    {">"}
                </button>
                ...
                <button
                    onClick={() => {
                        const lastGroup = Math.floor((maxPage - 1) / PAGE_BUTTONS_PER_GROUP);
